...
export async function POST(request: Request) {
  try {
    ...
    const { service, selections, result, lead } = data;
    const serviceLabel = service === "water-heater" ? "Water Heater" : "Whole-Home Repipe";

    // Build line items HTML
    const lineItemsHtml = result.lineItems
      .map(
        (item: { label: string; range: { min: number; max: number } }) =>
          `<tr>
            <td style="padding: 6px 0; color: #666;">${item.label}</td>
    ...
    const selectionsHtml = Object.entries(selections)
      ...
      .map(
        ([k, v]) =>
          `<p style="margin: 4px 0;"><strong>${k.replace(/([A-Z])/g, " $1").replace(/^./, (s: string) => s.toUpperCase())}:</strong> ${Array.isArray(v) ? v.join(", ") : v}</p>`
      )
      .join("");

    // Notes HTML
    const notesHtml = result.notes
      .map((n: string) => `<li style="margin: 4px 0; color: #666;">${n}</li>`)
      .join("");
